...
public class GroupUtils {
    ...
    public static class LoadMoreData<T> {
         ...
         * @return
         */
        public void setFetchResult(CursorResult<T> data) {
            fetchData.clear();
    ...
    static boolean isAdmin(Group group, String id) {
    ...
    static boolean isCurrentOwner(Group group) {
        return ChatClient.getInstance().getCurrentUser().equals(group.getOwner());
    }

    /**
     * judge whether current user can invite member join group
     */
    public static boolean isCanAddMember(Group group) {
        if (group.isMemberAllowToInvite() ||
                isAdmin(group, ChatClient.getInstance().getCurrentUser()) ||
