package io.agora.chatdemo.user.model.parse;
...
public class ParseManager {
    ...
    public void getContactsInfo(List<String> hxIdList, final ValueCallBack<List<UserEntity>> callback) {
        ParseQuery<ParseObject> pQuery = ParseQuery.getQuery(CONFIG_TABLE_NAME);
        ...
        pQuery.findInBackground(new FindCallback<ParseObject>() {
            ...
            @Override public void done(List<ParseObject> arg0, ParseException arg1) {
                if (arg0 != null) {
                    List<UserEntity> mList = new ArrayList<UserEntity>();
                    for (ParseObject pObject : arg0) {
                        UserEntity user = new UserEntity(pObject.getString(CONFIG_USERNAME));
    ...
    public void asyncGetCurrentUserInfo(final ValueCallBack<UserEntity> callback) {
        final String username = ChatClient.getInstance().getCurrentUser();
        asyncGetUserInfo(username, new ValueCallBack<UserEntity>() {
                ...
                if (error == ParseException.OBJECT_NOT_FOUND) {
                    ...
                    pUser.saveInBackground(new SaveCallback() {
                        ...
                        @Override public void done(ParseException arg0) {
                            if (arg0 == null) {
                                callback.onSuccess(new UserEntity(username));
                            }
                        }
                    });
                } else {
    ...
    public void asyncGetUserInfo(final String username, final ValueCallBack<UserEntity> callback) {
        ...
        pQuery.getFirstInBackground(new GetCallback<ParseObject>() {
            ...
            @Override public void done(ParseObject pUser, ParseException e) {
                if (pUser != null) {
