...
import io.agora.easeui.widget.EaseMessageListView.MessageListItemClicksListener;
import io.agora.easeui.widget.chatrow.EaseChatRow;
import io.agora.easeui.widget.chatrow.EaseChatRowFile;
import io.agora.easeui.widget.chatrow.EaseChatRowImage;
import io.agora.easeui.widget.chatrow.EaseChatRowLocation;
import io.agora.easeui.widget.chatrow.EaseChatRowText;
import io.agora.easeui.widget.chatrow.EaseChatRowVideo;
import io.agora.easeui.widget.chatrow.EaseChatRowVoice;
...
public class EaseMessageListAdapter extends BaseAdapter {
    ...
    public EaseMessageListAdapter(Context context, String username, int chatType,
        ...
        this.conversation = ChatClient.getInstance()
                .chatManager()
    ...
    Handler handler = new Handler() {
        private void refreshList() {
            ...
            messages = (ChatMessage[]) conversation.getAllMessages().toArray(new ChatMessage[0]);
    ...
    /**
     * get number of message type, here 14 = (ChatMessage.Type) * 2
     */
    ...
    public int getItemViewType(int position) {
        ...
        if (message.getType() == ChatMessage.Type.TXT) {
            if (message.getBooleanAttribute(EaseConstant.MESSAGE_ATTR_IS_BIG_EXPRESSION, false)) {
                return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_EXPRESSION
                        : MESSAGE_TYPE_SENT_EXPRESSION;
            }
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_TXT
                    : MESSAGE_TYPE_SENT_TXT;
        }
        if (message.getType() == ChatMessage.Type.IMAGE) {
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_IMAGE
                    : MESSAGE_TYPE_SENT_IMAGE;
        }
        if (message.getType() == ChatMessage.Type.LOCATION) {
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_LOCATION
                    : MESSAGE_TYPE_SENT_LOCATION;
        }
        if (message.getType() == ChatMessage.Type.VOICE) {
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_VOICE
                    : MESSAGE_TYPE_SENT_VOICE;
        }
        if (message.getType() == ChatMessage.Type.VIDEO) {
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_VIDEO
                    : MESSAGE_TYPE_SENT_VIDEO;
        }
        if (message.getType() == ChatMessage.Type.FILE) {
            return message.direct() == ChatMessage.Direct.RECEIVE ? MESSAGE_TYPE_RECV_FILE
    ...
    protected EaseChatRow createChatRow(Context context, ChatMessage message, int position) {
